...
| R2 | SemVer из conventional-commits: сначала `cog bump --auto --dry-run`, затем `cog bump --auto --annotated "vX.Y.Z"` → annotated tag + CHANGELOG | ⏸ показать тэг/diff/notes → approval |
| R2.5 | `node hooks/release-preflight.js --tag vX.Y.Z --base origin/main` — clean tree, tag→HEAD, remote tag absent, project/package versions = tag | |
| R3 | `git push origin <branch> && git push origin vX.Y.Z` | ⏸ только после явного «да» |
| R4 | если release workflow настроен: `gh run watch` — релизный workflow зелёный, без skipped-шагов | |
...
`release-preflight.js` намеренно валит релиз, если tag/changelog уже готовы, а проектный
manifest всё ещё сообщает старую версию (например `.csproj <Version>`). Если у проекта
нет version manifest, preflight даёт WARN; тогда версия бинарника всё равно проверяется в R6.
